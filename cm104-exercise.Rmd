...
library(repurrrsive) # it is a list of lists.
```
...
a <- c(1,2,3)
b <- c(4,5,6)
map2(a, b, function(x, y) x*y) # map2 coz of 2 variables
```
```{r}
map2(a, b, ~ .x * .y) # same function like before.
```
```{r}
map2(a, b, `*`) # shortcut
```
```{r}
map2_dbl(a, b, `*`) # in vector form for the elements are not vectorized
```
...
```{r} 
# p here means any number more than 2
a <- c(1,2,3)
b <- c(4,5,6)
c <- c(7,8,9)
...
```
```{r}
pmap(list(a, b, c), ~ ..1 * ..2 * ..3) # instead of .x,.y it is now ..1, ..2
```
```{r}
...
```{r}
# need to have same vector length
