...
# this is more friendly because it warns about the different levels.
```
...
# SHOWS THE MISSING CONTINENTS TOO
```
...
cont %>% 
  fct_infreq() %>% 
  qplot()
# bars ordered in order of frequency
```
...
# fct_reorder2 is looking at the country and the last max value then putting it first in the legend. for reverse, maybe use fct_rev
```
...
```{r}
# for ordering the way I want to. No ordering is responsible but my wish.
...
```{r}
# to place Syria after 2 entries.
gap_asia_2007$country %>% 
...
```{r}
# collapse = combining more than one level
gap_2007 <- gapminder %>% 
  filter(year == 2007)
```
...
cont %>% 
  ...
  qplot()
# the y-axis is the number of entries/countries that appeared in the data frame for the continent
```
...
```
```{r}
# keep the top 2 and lump the others
cont %>% 
  fct_lump(n=2) %>% 
  qplot()
```
